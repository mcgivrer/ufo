    ...
    */
    ...
    drawDebugObject(c, o) {
        if (this.game.debug > 1 && o.debug && o.debug>1) {
            ...
            dbg.push({
                attr: "acc",
                value: Math.round(o.acceleration.x) + "," + Math.round(o.acceleration.y)
            })
    ...
    renderDebugInfoForGO(c, o, props) {
        ...
        c.fillRect(o.position.x + props.px - 4, o.position.y + props.py, 100, 12 * (props.text.length + 1))

        // draw BoundingBox
        c.strokeRect(o.position.x - o.size.width / 2, o.position.y - o.size.height / 2, o.size.width, o.size.height)

        // draw text information
        c.fillStyle = props.dbgTxtColor
        var dx = 0
        props.text.forEach(ld=>{
            c.fillText(ld.attr + ":" + ld.value, o.position.x + props.px, o.position.y + props.py + ((dx + 1) * 12));
            dx += 1
        }
        )
    }
